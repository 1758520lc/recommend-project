...
def process_dataset(data_path='dataset/user_behavior.csv'):
    ...
    positive_triple_set={(user_id,item_id,1)for row in dec_03_data}
    negtive_triple_set=set()
    for row in enumerate(f):
        user_id,item_id,action_id=row[0],row[1],row[3]
        while True:
            neg_items_id=random.choice(item_id)
            if(user_id,neg_items_id,1) not in positive_triple_set:
                negtive_triple_set.add((user_id,item_id,0))
                break
